...
class SiswaController extends Controller
{
  public function __construct()
  {
    ...
    // $this->middleware(['permission:siswa.index|siswa.create|siswa.edit|siswa.delete|siswa.detail']);
  ...
  public function aturanujian()
  {
    if (auth()->user()->status == 'G' or auth()->user()->status == 'A') {
      $user = User::where('id', auth()->user()->id)->first();
      $kelas = Kelas::get();
      return view('siswa.aturan', compact('user', 'kelas'));
    } else {
      return redirect('/home');
    }
  }
  ...
  public function ujian()
  {
    ...
    ->where('distribusisoals.id_kelas',auth()->user()->id_kelas)
    ->where('paket','LATIHAN')
    ->get();
    return view('halaman-siswa.ujian', compact('user', 'pakets'));
  }
  public function ujian_uts()
  {
    $user = User::where('id', auth()->user()->id)->first();
    $pakets = DB::table('distribusisoals')
    ->join('soals','distribusisoals.id_soal','soals.id')
    ...
    ->where('paket','UTS')
    ->get();
    return view('halaman-siswa.uts', compact('user', 'pakets'));
  }
  public function ujian_uas()
  {
    $user = User::where('id', auth()->user()->id)->first();
    $pakets = DB::table('distribusisoals')
    ->join('soals','distribusisoals.id_soal','soals.id')
    ...
    ->where('paket','UAS')
    ->get();
    return view('halaman-siswa.uas', compact('user', 'pakets'));
  }
